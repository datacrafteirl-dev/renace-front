    ...
    document.querySelectorAll(".init-swiper").forEach(function(swiperElement) {
      ...
      try {
        ...
        if (cfgEl && cfgEl.innerHTML) {
          ...
          // Config por defecto segura si no hay .swiper-config
          config = { slidesPerView: 1, loop: false, speed: 500 };
        }
      } catch (e) {
        // Evitar que un slider sin config rompa los demás
        console.warn('Swiper config inválida, usando valores por defecto', e);
        config = { slidesPerView: 1, loop: false, speed: 500 };
      }
  ...
  function initCustomLightbox() {
      ...
      ">
        ...
        ">
          ...
          <button id="lightbox-close" style="
            ...
            z-index: 10000;
          " onmouseover="this.style.background='rgba(198, 32, 118, 0.95)'; this.style.color='white'; this.style.transform='scale(1.1)'" onmouseout="this.style.background='rgba(255, 255, 255, 0.95)'; this.style.color='#333'; this.style.transform='scale(1)'">×</button>
        </div>
      </div>
    `;
    
    // Agregar el HTML al body si no existe
    ...
    function openLightbox(src) {
      ...
      img.onerror = function() {
        lightboxImage.src = 'data:image/svg+xml;base64,';
        lightboxImage.style.opacity = '1';
        lightboxImage.style.filter = 'blur(0px)';
        lightboxImage.style.transition = 'opacity 0.3s ease, filter 0.3s ease';
      };
    ...
    // Hacer que todas las imágenes con clase glightbox sean clickeables
    function attachLightboxToImages() {
      document.querySelectorAll('.glightbox').forEach(img => {
    ...
    // Re-inicializar cuando se carguen nuevas imágenes (para contenido dinámico)
        ...
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach(function(node) {
            if (node.nodeType === 1) { // Element node
              ...
              // También buscar imágenes dentro del nodo agregado
              const images = node.querySelectorAll && node.querySelectorAll('.glightbox');
              if (images) {
                images.forEach(img => {
                  img.style.cursor = 'pointer';
                  img.addEventListener('click', function(e) {
    ...
    console.log('Lightbox personalizado inicializado correctamente');
  }
  
  // Inicializar lightbox personalizado
  initCustomLightbox();
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('img').forEach(img => {
      img.addEventListener('error', function() {
        ...
        // Mostrar imagen placeholder si falla la carga
